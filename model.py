...
# Добавили сохранение файла

def add_data(new: str) -> dict[str, str]:
    with open( 'a', encoding = 'utf-8') as file:
        name = input('Введите имя: ')
        phone_number = input('Введите номер телефона: ')
        commentik = input('Комментарий: ')
        file.write(f'{name} | {phone_number} | {commentik}')
        print(f'Добавлен файл: {name} | {phone_number} | {commentik}')
